...
let { isEmpty, isValidObjectId, isValidISBN, isValidExcerpt, isValidName, isValidDate } = validation;
...
const createBook = async function (req, res) {
  try {
    ...
    if (!isValidExcerpt(excerpt)) {
      return res.status(400).send({status: false,message: "excerpt should contain alphabates only"});
    }
...
const getBooks = async function (req, res) {
  try {
    ...
    if (Object.keys(queryData) == 0) {
      let blog = await bookModel.find({userId: validUserId, isDeleted: false});
    ...
    const bookData = await bookModel.find( addObj ).select(showData).sort({ title: 1 });
...
const getBooksById = async function (req, res) {
  try {
    ...
    const checkBook = await bookModel.findOne({ _id: bookId, isDeleted: false}).select({ __v: 0, ISBN: 0 });
    if (!checkBook) {
      return res.status(404).send({ status: false, message: "No Book Found" });
    }
    const checkReview = await reviewModel.find({bookId: bookId,isDeleted: false}).select({__v:0, createdAt: 0, updatedAt: 0, isDeleted: 0});

    checkBook._doc.reviewsData = checkReview;

    return res.status(200).send({ status: true, message: "BooksList", data: checkBook });
  } catch (err) {
...
const updateBook = async function (req, res) {
  try {
    ...
    let checkTitle = await bookModel.findOne({ title: title });
    if (checkTitle) {
      return res.status(400).send({status: false, message: "title is already exists please provide unique title"});
    }
    if (title) {
    ...
    let checkISBN = await bookModel.findOne({ ISBN: ISBN });
    if (checkISBN) {
      return res.status(400).send({status: false,message: "ISBN already exists please provide unique ISBN"});
    }
    if (ISBN) {
      if (!isValidISBN(ISBN)) {
        return res.status(400).send({status: false,message: "ISBN should follow 10 to 13 digit numbers only"});
      }
    }

    if (releasedAt) {
      if (!isValidDate(releasedAt)) {
        return res.status(400).send({status: false,message: "Please provide the release date in the format of 'YYYY-MM-DD'"});
      }
    }
    releasedAt = Date.now();
    let updatedBooks = await bookModel.findOneAndUpdate({ _id: bookId },{ title: title, excerpt: excerpt, releasedAt: releasedAt, ISBN: ISBN },{ new: true });
    res.status(200).send({status: true,message: "Successfully updated book Details",data: updatedBooks});
  } catch (err) {
...
const deleteBook = async function (req, res) {
  try {
    ...
    res.status(200).send({status: true,msg: "Books data has been deleted successfully",data: checkBook});
  } 
  catch (error) {
