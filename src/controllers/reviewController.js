...
let {isEmpty,isValidName, isValidObjectId} = require("../validator/validator");
...
const createReview = async function (req, res) {
  try {
    ...
    if (!bookId) {
      return res.status(404).send({ status: false, message: "Enter a valid book id" });
    }
    ...
    if (reviewedBy) {
      if (!isEmpty(reviewedBy)) {
        return res.status(400).send({ status: false, message: "Reviewed Name must be present" });
      }
    ...
    if (!isValidObjectId(data.bookId)) {
      return res.status(400).send({ status: false, message: "It should be a valid bookId" });
    }
    ...
    return res.status(201).send({status: true, message: "Review added successfully", data: checkBooks._doc });
  } catch (err) {
...
const updateReview = async (req, res) => {
  try {
    ...
    let updatedReview = await reviewModel.findOneAndUpdate({ _id: reviewId, bookId: bookId, isDeleted: false },{ review: review, rating: rating, reviewedBy: reviewedBy },{ new: true });

    let { ...updatedBook } = checkBook;
    updatedBook._doc.reviewesData = updatedReview;
    res.status(200).send({status: true, message: "Successfully updated Review Details", data: updatedBook._doc});
  } catch (err) {
    return res.status(500).send({ status: false, message: err.message });
  }
};
...
const deleteReview = async function (req, res) {
  try {
    ...
    let updateReviewDetails = await reviewModel.findOneAndUpdate({ _id: reviewId, isDeleted: false },{ $set: { isDeleted: true, deletedAt: Date.now() } }, { new: true });

    await bookModel.findOneAndUpdate({ _id: bookId, isDeleted: false }, { $inc: { reviews: -1 } } );

    res.status(200).send({status: true,msg: "Review data has been deleted successfully", data: updateReviewDetails});
  } catch (error) {
